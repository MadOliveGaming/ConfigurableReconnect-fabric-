...
public record MenuStateUpdateMessage(int elementType, String name, Object elementState) implements CustomPacketPayload {
	public static final Type<MenuStateUpdateMessage> TYPE = new Type<>(Identifier.fromNamespaceAndPath(ConfigurableReconnectMod.MODID, "menustate_update"));
	...
	public static void handleMenuState(final MenuStateUpdateMessage message, final ServerPlayNetworking.Context context) {
		...
		context.server().execute(() -> {
			if (context.player().containerMenu instanceof ConfigurableReconnectModMenus.MenuAccessor menu) {
				menu.getMenuState().put(message.elementType + ":" + message.name, message.elementState);
				if (Minecraft.getInstance().gui.screen() instanceof ConfigurableReconnectModScreens.FabricScreenAccessor accessor) {
