...
import madolivegaming.configurablereconnect.network.MenuStateUpdateMessage;
import madolivegaming.configurablereconnect.ConfigurableReconnectMod;

import java.util.Map;

public class ConfigurableReconnectModMenus {
	public static MenuType<ConfigurableReconnectGUIMenu> CONFIGURABLE_RECONNECT_GUI;

	public static void load() {
		CONFIGURABLE_RECONNECT_GUI = register("configurable_reconnect_gui", ConfigurableReconnectGUIMenu::new);
		ConfigurableReconnectGUIMenu.screenInit();
	...
	public interface MenuAccessor {
		...
		default void sendMenuStateUpdate(Player player, int elementType, String name, Object elementState, boolean needClientUpdate) {
			...
			if (player instanceof ServerPlayer serverPlayer) {
				...
				if (Minecraft.getInstance().gui.screen() instanceof ConfigurableReconnectModScreens.FabricScreenAccessor accessor && needClientUpdate)
	...
	private static <M extends AbstractContainerMenu> MenuType<M> register(String registryname, MenuType.MenuSupplier<M> element) {
		return Registry.register(BuiltInRegistries.MENU, Identifier.fromNamespaceAndPath(ConfigurableReconnectMod.MODID, registryname), new MenuType<>(element, FeatureFlags.DEFAULT_FLAGS));
	}
}